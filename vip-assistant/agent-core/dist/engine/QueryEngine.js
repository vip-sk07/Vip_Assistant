...
import { costFromUsage } from "./modelClient.js";
...
export class QueryEngine {
     ...
     */
    async *submitMessage(userContent, signal) {
    ...
    async *runLoop(signal) {
        ...
        while (this.turnCount < maxTurns) {
            ...
            if (this.config.maxBudgetUsd !== undefined &&
                ...
                yield this.errorEvent("BUDGET_EXCEEDED", `Cost $${this.accumulatedCostUsd.toFixed(4)} reached limit $${this.config.maxBudgetUsd}`, false);
                return;
            }
            ...
            for (const toolUse of toolUseBlocks) {
                ...
                const ctx = {
                    sessionId: this.config, // injected externally in production
                    cwd: this.config.cwd,
    ...
    async executeToolWithPermissions(toolUse, ctx) {
        const tool = this.config.tools.get(toolUse.name);
        if (!tool) {
            return {
    ...
    errorEvent(code, message, retryable) {
        return {
            type: "error",
    ...
    // ── Public accessors ──────────────────────────────────────────────────────
    get totalCostUsd() { return this.accumulatedCostUsd; }
    get totalUsage() { return { ...this.accumulatedUsage }; }
    get history() { return [...this.messages]; }
    async attemptSelfHealing(toolUse, result, ctx) {
...
Generate the JSON response:`;
    ...
    async attemptDependencySelfHealing(toolUse, result, ctx) {
        const errorOutput = typeof result.content === "string" ? result.content : "";
...
function parseErrorLocation(content) {
...
function parseMissingDependency(output) {
...
async function getLlmCompletion(model, systemPrompt, userPrompt, signal) {
