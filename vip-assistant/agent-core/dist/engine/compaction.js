...
// ─────────────────────────────────────────────────────────────────────────────
export async function compactHistory(messages, params) {
    ...
    if (messages.length <= head + tail + 2)
        return messages;
...
// ─── Summary generation ───────────────────────────────────────────────────────
async function summariseMessages(messages, params) {
    ...
    const transcript = messages
        ...
        const text = m.content
            ...
            if (b.type === "tool_result")
                return `[Tool result: ${typeof b.content === "string" ? b.content.slice(0, 500) : "..."}]`;
            return "";
        })
            .filter(Boolean)
            .join("\n");
